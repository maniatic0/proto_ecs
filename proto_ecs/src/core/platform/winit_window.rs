...
use proto_ecs::core::window::{Window, WindowProperties, WindowPtr};
...
pub fn winit_window_builder(props: WindowProperties) -> WindowPtr {
...
impl Window for WinitWindow {
    ...
    fn poll_events(&mut self) -> Vec<crate::prelude::Event> {
        let mut events = vec![];
        self.event_loop.pump_events(Some(Duration::ZERO), |event, event_loop|{
            events.push(to_event(event));
        });
