...
macro_rules! get_context {
    ($i:ident) => {
        let __context__ = proto_ecs::core::platform::opengl::opengl_render_backend::OPENGL_CONTEXT.read();
        let $i = __context__.as_ref().expect("Opengl Context not yet initialized!");
    };
}
...
impl RenderAPIBackend for OpenGLRenderBackend {
    fn create() -> RenderAPIBackendPtr {
        ...
        *context = Some(OpenGLContext { gl: winit_window.get_glow_context() });
        Box::new(OpenGLRenderBackend {
    ...
    fn draw_indexed(&mut self, vertex_array: &VertexArrayPtr) {
        ...
        unsafe {
            context.gl.draw_elements(
                glow::TRIANGLES,
                vertex_array.get_index_buffer().get_count() as i32,
                glow::UNSIGNED_INT,
                0,
            );
        }
