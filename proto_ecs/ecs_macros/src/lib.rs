...
pub fn local_system(
    ...
    for arg in sig.inputs.iter() {
        ...
        match arg {
            syn::FnArg::Typed(pt) => match (*pt.ty).clone() {
                syn::Type::Reference(r) => match (*r.elem).clone() {
                    syn::Type::Path(p) => arg_types.push(p),
                    _ => {
                        return quote!{compile_error!("Systems should only expect DataGroups as input")}.into();
                    }
                },
                _ => {
                    return quote!{compile_error!("Systems should only expect DataGroups as input")}.into();
            ...
            _ => {
                return quote!(compile_error!(
                    "Systems should only expect DataGroups as input"
                ))
                .into();
            }
        }
    ...
    for i in 0..arg_types.len() {
        new_function += format!(
            ...
            i,
            i,
            arg_types[i].path.clone().into_token_stream().to_string()
        )
